...
const StatTile = ({ value, label }) => (
  <div className="bg-white rounded-xl border border-[rgba(22,25,22,0.12)] p-5">
    <p className="text-3xl font-bold text-gp-midnight">{value}</p>
    <p className="text-xs text-gp-fl3 mt-1">{label}</p>
  </div>
);

const SectionLabel = ({ children }) => (
  <p className="text-xs font-semibold text-gp-fl3 uppercase tracking-widest mb-3">{children}</p>
);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {/* WIP Status */}
        <div className="bg-white rounded-xl border border-[rgba(22,25,22,0.12)] p-5 flex flex-col gap-3">
          <p className="text-sm font-semibold text-gp-fl1">Current WIP status</p>
        ...
        {/* Upcoming meetings */}
        <div className="bg-white rounded-xl border border-[rgba(22,25,22,0.12)] p-5 flex flex-col gap-3">
          <p className="text-sm font-semibold text-gp-fl1">Upcoming meetings</p>
          {approved.length === 0 ? (
            <p className="text-xs text-gp-fl3">No upcoming meetings.</p>
          ) : (
            <div className="flex flex-col gap-2">
              {approved.map(r => (
                <div key={r.id} className="border-b border-[rgba(22,25,22,0.06)] pb-2 last:border-0 last:pb-0">
                  <p className="text-sm font-medium text-gp-fl1">{r.title}</p>
        ...
        {/* Tasks due soon */}
        <div className="bg-white rounded-xl border border-[rgba(22,25,22,0.12)] p-5 flex flex-col gap-3">
          <p className="text-sm font-semibold text-gp-fl1">Tasks due soon</p>
          {dueSoon.length === 0 ? (
            <p className="text-xs text-gp-fl3">No upcoming deadlines.</p>
          ) : (
            <div className="flex flex-col gap-2">
              {dueSoon.map(t => (
                <div key={t.id} className="border-b border-[rgba(22,25,22,0.06)] pb-2 last:border-0 last:pb-0">
                  <p className="text-sm font-medium text-gp-fl1">{t.title}</p>
                  <p className="text-xs text-gp-fl3">Due {formatDate(t.dueDate)}</p>
                </div>
              ))}
            </div>
          )}
...
function LeadDashboard({ currentUser, tasks, meetingRequests }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Team workload grid */}
      <div className="bg-white rounded-xl border border-[rgba(22,25,22,0.12)] p-5 flex flex-col gap-4">
        <SectionLabel>Team workload — {myTeam?.name}</SectionLabel>
        {teamMembers.map(member => {
          const active = getActiveTaskCount(member.id, tasks);
          return (
            <div key={member.id} className="flex items-center gap-3 text-sm">
              <div className="w-7 h-7 rounded-full bg-gp-cream text-gp-midnight text-xs font-bold flex items-center justify-center shrink-0">
                {member.avatar}
              </div>
              <div className="w-32 min-w-0">
                <p className="text-sm font-medium text-gp-fl1 truncate">{member.name}</p>
                <p className="text-xs text-gp-fl3 truncate">{member.title}</p>
              </div>
              <div className="flex-1 bg-gp-cream rounded-none h-2">
                <div
                  className={`h-2 rounded-none transition-all ${barColor(active)}`}
                  style={{ width: `${Math.min((active / 5) * 100, 100)}%` }}
                />
              </div>
              <span className="text-xs text-gp-fl3 w-16 shrink-0">{active} active</span>
            </div>
          );
        })}
        <div className="pt-3 border-t border-[rgba(22,25,22,0.08)] flex gap-5 text-xs">
      ...
      <div className="bg-white rounded-xl border border-[rgba(22,25,22,0.12)] p-5 flex flex-col gap-3">
        ...
        {pending.length === 0 ? (
          <p className="text-xs text-gp-fl3">No pending requests.</p>
        ) : (
          <div className="flex flex-col gap-2">
            {pending.map(r => {
              const requester = getUserById(r.requestedBy);
              return (
                <div key={r.id} className="flex items-start justify-between gap-3 py-2 border-b border-[rgba(22,25,22,0.06)] last:border-0">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gp-fl1">{r.title}</p>
                    {requester && <p className="text-xs text-gp-fl3 mt-0.5">{requester.name}</p>}
                    {r.reason && <p className="text-xs text-gp-fl3 italic mt-0.5 truncate">{r.reason}</p>}
                  </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {TEAMS.map(team => {
            ...
            return (
              <div key={team.id} className="bg-white rounded-xl border border-[rgba(22,25,22,0.12)] p-5 flex flex-col gap-3">
                <div>
                  <p className="text-sm font-semibold text-gp-fl1">{team.name}</p>
                  {lead && <p className="text-xs text-gp-fl3">Lead: {lead.name}</p>}
                </div>
                {members.map(member => {
                  const active = getActiveTaskCount(member.id, tasks);
                  return (
                    <div key={member.id} className="flex items-center gap-3 text-sm">
                      ...
                      <span className="w-28 text-xs text-gp-fl1 truncate">{member.name}</span>
                      <div className="flex-1 bg-gp-cream rounded-none h-2">
                        <div
                          className={`h-2 rounded-none transition-all ${barColor(active)}`}
                          style={{ width: `${Math.min((active / 5) * 100, 100)}%` }}
                      ...
                      <span className="text-xs text-gp-fl3 w-12 shrink-0">{active}</span>
                    </div>
                  );
                })}
              </div>
      ...
      <div className="bg-white rounded-xl border border-[rgba(22,25,22,0.12)] p-5 flex flex-col gap-3">
        ...
        {deptCounts.map(({ dept, count }) => (
          <div key={dept} className="flex items-center gap-3 text-sm">
            <span className="w-28 text-xs text-gp-fl1">{dept}</span>
            <div className="flex-1 bg-gp-cream rounded-none h-2">
              <div
                className="bg-indigo-400 h-2 rounded-none transition-all"
                style={{ width: `${(count / maxDept) * 100}%` }}
              />
            </div>
            <span className="text-xs text-gp-fl3 w-6">{count}</span>
          </div>
        ))}
        {unassigned > 0 && (
          <div className="flex items-center gap-3 text-sm">
            <span className="w-28 text-xs text-gp-fl3 italic">Unassigned</span>
            <div className="flex-1 bg-gp-cream rounded-none h-2">
              <div
                className="bg-gray-300 h-2 rounded-none transition-all"
                style={{ width: `${(unassigned / maxDept) * 100}%` }}
              />
            </div>
            <span className="text-xs text-gp-fl3 w-6">{unassigned}</span>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto flex flex-col gap-6">
      ...
      <div className="gp-wayfinder">
        <p className="text-xs font-semibold text-gp-fl3 uppercase tracking-widest">Welcome back</p>
        <h1 className="text-3xl font-bold text-gp-midnight mt-1">{currentUser.name}</h1>
        <p className="text-sm text-gp-fl2 mt-1">Your personalized meeting and task overview.</p>
      </div>
